...
impl ExampleMessage {
    pub fn new(id: u32, val: f32, name: String, nums: Vec<i32>) -> Self {
        ...
        let nums_length = nums.len() as u32;
        Self {
            id,
            val,
            name,
    ...
    pub fn serialize(&self) -> Vec<u8> {
        ...
        buffer.extend(&self.name_length.to_le_bytes());
        buffer.extend(self.name.as_bytes());
        buffer.extend(&self.nums_length.to_le_bytes());
    ...
    pub fn deserialize(buffer: &[u8]) -> io::Result<Self> {
        ...
        let mut nums = Vec::with_capacity(nums_length as usize);
